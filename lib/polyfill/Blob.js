...
var _fs=require('../fs.js');var _fs2=_interopRequireDefault(_fs);
...
Blob=function(_EventTarget){_inherits(Blob,_EventTarget);_createClass(Blob,[{key:'blobPath',get:function get()
...
{
return this._ref;
}}],[{key:'clearCache',value:function clearCache(){return _fs2.default.unlink(blobCacheDir).then(function(){return _fs2.default.mkdir(blobCacheDir);});}},{key:'build',value:function build(data,cType){return new Promise(function(resolve,reject){new Blob(data,cType).onCreated(resolve);});}},{key:'setLog',value:function setLog(
...
function Blob(data,cType,defer){_classCallCheck(this,Blob);var _this=_possibleConstructorReturn(this,(Blob.__proto__||Object.getPrototypeOf(Blob)).call(this));_this.isRNFetchBlobPolyfill=true;_this.multipartBoundary=null;_this._ref=null;_this._blobCreated=false;_this._onCreated=[];_this._closed=false;
...
if(!data)
data='';
if(data.isRNFetchBlobPolyfill){(function(){
log.verbose('create Blob cache file from Blob object');
var size=0;
_this._ref=String(data.getRNFetchBlobRef());
var orgPath=_this._ref;

p=_fs2.default.exists(orgPath).
then(function(exist){
if(exist)
return _fs2.default.writeFile(orgPath,data,'uri').
then(function(size){return Promise.resolve(size);}).
catch(function(err){
...
if(defer)
return _possibleConstructorReturn(_this);else
{
p=_fs2.default.stat(_orgPath).
then(function(stat){
...
p=_fs2.default.writeFile(_this._ref,data,encoding).
then(function(size){
...
{
data=data.toString();
p=_fs2.default.writeFile(_this._ref,data,'utf8').
then(function(size){return Promise.resolve(size);});
}
...
}_createClass(Blob,[{key:'onCreated',value:function onCreated(
...
this._onCreated.push(fn);else
{
fn(this);
}
return this;
}},{key:'markAsDerived',value:function markAsDerived()

{
this._isDerived=true;
}},{key:'getRNFetchBlobRef',value:function getRNFetchBlobRef()
...
start,end){var contentType=arguments.length>2&&arguments[2]!==undefined?arguments[2]:'';
if(this._closed)
...
_fs2.default.slice(this._ref,resPath,start,end).then(function(dest){
...
return result;
}},{key:'readBlob',value:function readBlob(
...
}},{key:'_invokeOnCreateEvent',value:function _invokeOnCreateEvent()
...
}},{key:'isDerived',get:function get(){return this._isDerived||false;}}]);return Blob;}(_EventTarget3.default);exports.default=Blob;
...
}.bind(args[_i]));};for(var _i in args){_loop(_i);
}
return p.then(function(){return Promise.resolve(size);});
}